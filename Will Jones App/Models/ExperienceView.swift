        ...
        ScrollView
        {
            VStack
            {
                ...
                VStack(alignment: .leading)
                {
                    ...
                    Text("Manage and administer a hybrid-cloud homelab environment consisting of over 25 VMs and devices using Ansible. Deployed multiple Docker containers, both in the cloud and in my homelab, to create my own search engine using Searxng, pi-hole with unbound for recursive DNS, and a Grafana dashboard, utilizing Prometheus, Telegraf, and InfluxDB, to monitor statistics about my home network and homelab, such as internet speed. Built a custom Telegraf Docker container to support IPMI to pull metrics from Dell PowerEdge and Apple Xserve servers. Built a custom Docker container to house the Apple RAID Admin tool for managing their Xserve RAID. Manage a Windows Deployment Server to create and deploy custom, as well as standard, Windows images to clients and VMs across my network and homelab. Deployed a pfSense firewall/router to better control and segment my homelab network from the rest of my network for improved security. Deployed a TrueNAS server on a Dell PowerEdge server to back up my personal data as well as the VMs for my homelab.")
                        .font(.footnote)
                        .padding(5)
